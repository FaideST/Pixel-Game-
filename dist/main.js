...
window.addEventListener("DOMContentLoaded", () => {
    ...
    function update() {
        ...
        for (const platform of allPlatforms) {
            ...
            const horizontalOverlap = playerX + playerWidth > platform.x &&
                playerX < platform.x + platform.w;
            const fallingOntoPlatform = playerPrevBottom <= platformTop &&
        ...
        const playerBottom = playerY + playerHeight;
        const playerPrevBottom = playerBottom - velocityY;
        const standingOnGoal = playerX + playerWidth > goal.x &&
    ...
    function draw() {
        ...
        for (let i = -3; i <= 1; i++) {
            ctx.drawImage(sky, 0, i * skyHeight - cameraY, canvas.width, skyHeight);
        }
        const currentMap = maps[currentMapIndex];
        ctx.drawImage(currentMap.bg, 0, -cameraY, canvas.width, canvas.height);
        // Platformen 
        for (const platform of platforms) {
            ctx.drawImage(platform.img, platform.x, platform.y - cameraY, platform.w, platform.h);
        }
        ctx.drawImage(goal.img, goal.x, goal.y - cameraY, goal.w, goal.h);
        // Items
