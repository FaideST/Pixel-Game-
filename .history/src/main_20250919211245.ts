...
const npcs = [ 
  {img: new Image(), x: 100, y: 0, w: 32, h: 32}, 
]

npcs[0].img.src ="assets/sprites/npcs/Doktor.png";

//pet
const pet = [ {img: new Image (), offsetX: -20, offsetY: 0, w: 24, h: 24, }
]
...
function update() {
    ...
    if (playerY - cameraY > canvas.height - 80) {
        ...
        pet[0].x = playerX + pet[0].offsetX;
        pet[0].y = playerY + pet[0].offsetY;
...
// Pet + UI zeichnen
    ...
    ctx.drawImage(pet[0].img, pet[0].x, pet[0].y - cameraY, pet[0].w, pet[0].h);
